...
\noindent 聚焦于\textbf{长程 LLM 智能体}研究：（1）在非平稳上下文条件下稳定多轮优化的智能体强化学习算法（FoldAct）；（2）超越终任务准确率、刻画智能体在交互过程中如何收集、修正与校准证据的评测方法（SeekBench，ICLR 2026）；以及（3）面向长时任务的可恢复、可介入、可持续运行智能体执行框架（字节跳动）。
\vspace{0.8em}
...
\section*{\color{sectioncolor}代表性研究成果}
    ...
    \noindent\textbf{\color{sectioncolor}第一作者} | 优化算法设计 \hfill \color{accentcolor}\textbf{arXiv, 2025} \\
    ...
    \begin{itemize}[leftmargin=*, label=\textcolor{accentcolor}{\textbullet}, itemsep=0.2em]
        ...
        \item 在复杂搜索智能体任务上实现5.19倍的训练加速，同时保持良好的长程决策质量。
    \end{itemize}
    \vspace{0.5em}
