...
\begin{itemize}[leftmargin=*]
    \item 建立"暴露$\rightarrow$利用$\rightarrow$失真"（Expose$\rightarrow$Exploit$\rightarrow$Mislead）的 \textbf{reward hacking 分析模型}，通过 8+ 个真实案例的代码级审计（NAS 查表、cache 刷分、hidden 数据泄漏等）归纳 benchmark 侧暴露面 taxonomy。设计 L0--L3 安全等级与自动化 hack 检测规则，落地为 benchmark 上线前审计 checklist。
    \item 负责研究型 Agent 未来评测体系的思考与构建。针对"AI 自我改进（RSI）"缺乏可信度量的问题，组织完成 8 大研究领域 20+ 个主流 benchmark 的系统调研与接入决策，提出 R0--R4 五级能力分级与配套证据标准；将评测从"一次性跑分"重构为覆盖任务设计、执行、测量、归一化、反馈、能力主张的 \textbf{六环节评测链路}，配套 hidden-test 成熟度模型与上线前完整性审计标准，使分数成为可辩护的能力证据。
\end{itemize}
