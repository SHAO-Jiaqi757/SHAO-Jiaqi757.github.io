...
\begin{center}
    {\fontsize{28}{34}\selectfont\textbf{\color{namecolor}邵佳琪}} \\ \vspace{0.3em}
    {\color{graytext}\fontsize{11}{13}\selectfont PhD, HKUST | Expected Graduation: June 2027} \\ \vspace{0.15em}
    {\color{graytext}\fontsize{11}{13}\selectfont Target: LLM Agent / RL / Agent Systems} \\ \vspace{0.4em}
    \begin{tabular}{c c c c c}
        \textbf{GitHub}: \href{https://github.com/SHAO-Jiaqi757}{\color{linkcolor}链接} &
        \textbf{主页}: \href{https://shao-jiaqi757.github.io/}{\color{linkcolor}链接} &
        \textbf{邮箱}: \href{mailto:jshaoaj@connect.ust.hk}{\color{linkcolor}jshaoaj@connect.ust.hk} &
        \textbf{Scholar}: \href{https://scholar.google.com/citations?user=P8MtRXMAAAAJ\&hl=zh-CN}{\color{linkcolor}链接}
    \end{tabular}
\end{center}
...
\textit{\color{graytext}电子与计算机工程学士 | 专业方向：计算机工程} \hfill \color{accentcolor}\textbf{2019 -- 2023}

\section*{\color{sectioncolor}研究方向}
\noindent 我的研究聚焦 \textbf{AI Agent 的可信评测与评测基础设施}，核心关注 benchmark 分数能否真实反映 Agent 的目标能力。围绕结果之外的\textbf{过程可靠性、评测协议有效性与评测规模化}，构建覆盖执行轨迹与评测环境的系统化评测方法：通过 \textbf{SeekBench} 将评测从最终准确率扩展至证据收集、错误恢复与置信校准等认知过程；通过 \textbf{HackDetect} 分析评测暴露与 reward hacking 对分数有效性的影响；进一步将相关方法沉淀为统一的 benchmark 接入与交付规范，为后续扩大任务规模、接入更多 Agent 及自动化评测奠定基础。

\section*{\color{sectioncolor}实习经历}
\subsubsection*{\color{itemcolor}腾讯混元大模型团队 | 高级研究员（青云实习计划）}
\noindent\textbf{\color{sectioncolor}RSI 评估与评测范式研究} \hfill \color{accentcolor}\textbf{2026年5月 -- 至今}
\begin{itemize}[leftmargin=*]
    \item \textbf{Reward Hacking 检测与评测安全：}建立"暴露$\rightarrow$利用$\rightarrow$失真"（Expose$\rightarrow$Exploit$\rightarrow$Mislead）分析框架，系统归纳 benchmark 侧暴露面 taxonomy；设计 L0--L3 评测安全等级与自动化 hack 检测规则，沉淀 benchmark 上线前审计 checklist。相关成果形成第一作者论文 \href{https://arxiv.org/abs/2607.22368}{\color{linkcolor}HackDetect}。
    \item \textbf{RSI 评测体系 0$\rightarrow$1 建设：}负责递归自我改进（RSI）评测体系从零构建，组织 8 大研究领域评测任务调研，对标 OpenAI Self-Improvement Suite，将评测对象从最终分数扩展至研究过程与协议有效性；制定覆盖 evaluator 隔离、hidden set 设计及 invalid/hack 判定的 benchmark 接入与交付标准，统一多来源任务的交付语义与评测口径，支撑分数可比、任务可组合与审计自动化，为评测规模从数十项扩展至数百项构建可持续演进的基础设施。
\end{itemize}
...
\subsubsection*{2. \color{itemcolor}Scale-Conditioned Agent Memory：智能体记忆的规模条件评测}
\noindent\textbf{\color{sectioncolor}第一作者} | 评测协议设计与系统实验 \hfill \color{accentcolor}\textbf{arXiv, 2026}
\noindent\href{https://arxiv.org/abs/2605.07313}{\color{linkcolor}[论文链接]}
\begin{itemize}[leftmargin=*]
    \item 识别固定快照准确率无法衡量记忆随无关会话累积是否仍可用，提出 evidence-preserving growth 的规模条件评测协议：固定任务证据并逐步增加无关会话，从预算约束可靠性、尾部调用开销、失败模式及可用规模边界四个维度进行诊断。
    \item 在 LongMemEval、LoCoMo 及 flat / planar / hierarchical 三类记忆接口上完成系统评测，揭示可扩展记忆能力取决于 Agent、接口、规模区间与交互预算；HippoRAG 的预算约束可靠性随无关会话增长下降 \textbf{16--20 个百分点}。
\end{itemize}

\subsubsection*{3. \color{itemcolor}FoldAct：面向长程大语言模型智能体的稳定优化方法}
...
\subsubsection*{4. \color{itemcolor}MorphAgent：自演化多智能体协作算法}
...
\begin{itemize}[leftmargin=*]
    ...
    \item 构建可扩展、可复用的评测基础设施，支持不同智能体、工具与数据集上的标准化比较，显著提升评测的可重现性与可解释性。
\end{itemize}
