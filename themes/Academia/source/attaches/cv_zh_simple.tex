...
\begin{itemize}[leftmargin=*]
    \item 研究 \textbf{RSI 评估}评测范式，建立"控制模型变量、隔离 Core 差异"的评估方法论，设计长程任务协议（单轮工具链 $\rightarrow$ 多轮迭代 $\rightarrow$ 跨会话持久 $\rightarrow$ 自主长程），系统量化命名空间保护强度、Hook 单调性、上下文压缩策略等 Core 机制差异，形成面向智能体基础设施的严格评测基准。
